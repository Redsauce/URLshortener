...
require 'vendor/autoload.php'; // include Composer stuff installed with: composer require "mongodb/mongodb=^1.0.0"
...
class RSMDB
{
     ...
     * Update
     *
     * Update any number of Documents (Rows in MySQL)  into the Collection.
     *
     * @param $data (Array) Contains the information in the following format (Note the double array is necessary even for a single element):
     * [['key' => 'value', 'key2' => 'value2'], ['key' => 'value3']]
     * @return MongoDB\InsertManyResult Which can be consulted with ->getInsertedIds to obtain an Array with the IDs of all successful inserts.
    ...
    public function Update($filter, $data, $options){
        $result = $this->collection->updateOne($filter, $data, $options);
        return $result;
    }
